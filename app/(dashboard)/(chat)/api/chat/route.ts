...
export async function POST(request: Request) {
  ...
  const result = await streamText({
    ...
    onFinish: async ({ responseMessages }) => {
      console.log(responseMessages, "[RESPONSE MESSAGES]");
      console.log(
        responseMessages?.map((content) => {
          console.log(content.role, content.content, "[ROLE,CONTENT]");
        }),
        "[RESPONSE MESSAGES]"
      );
      if (session.user?.id) {
        try {
          ...
          console.log(responseMessagesWithoutIncompleteToolCalls, [
            "NO responseMessagesWithoutIncompleteToolCalls",
          ]);
