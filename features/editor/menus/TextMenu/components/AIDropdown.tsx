...
} from "@/components/ui/dropdown";import { Button } from "@/components/ui/button";
import {
  Wand2,
  SpellCheck,
  PenLine,
...
export function AIDropdown({ editor }: AIDropdownProps) {
  ...
  const handleAIOperation = async (operation: string, language?: string) => {
    ...
    try {
      ...
      const result = await model.generateContent(`${prompt} provide the answer in plain text.`);
